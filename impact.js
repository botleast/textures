...
`);
	...
	addModification('document.addEventListener("contextmenu",m=>m.preventDefault());', /*js*/`
		...
		(function() {
			...
			// Creative Mode
			new Module("Creative", function(callback) {
    			if (callback) {
        			let lastUpdate = Date.now();
        			tickLoop["Creative"] = function() {
            			// Check if 0.5 seconds have passed
            			if (Date.now() - lastUpdate >= 500) {
            	    		// Give creative abilities
            	    			if (player) {
            	        			// Enable flight
            	    			    ...
            	    			    // Instant break
            	    			    if (playerControllerDump) {
            	    			        playerControllerDump.breakBlockInstantly = true;
            	    			    }

            	    			    // Unlimited reach
            	    			    if (playerControllerDump) {
            	    			        playerControllerDump.blockReachDistance = 100;
            	    			    }

            	    			    // Creative inventory
            	    			    if (player.inventory) {
            	    			        player.inventory.setCreative();
            	    			    }
            	    			}
        			...
        			delete tickLoop["Creative"];
        			// Revert abilities when module is turned off
        			if (player) {
            			// Disable flight
            			...
            			// Revert break speed
            			if (playerControllerDump) {
            			    playerControllerDump.breakBlockInstantly = false;
            			    playerControllerDump.blockReachDistance = 5;
            			}
        			}
    			}
			}, "Player");
